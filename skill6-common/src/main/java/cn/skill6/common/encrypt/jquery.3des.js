/**
 * @Desc 3DES加密工具, 使用方法：必须导入JQuery,然后使用$.方法名(参数...)调用，如$.tripleDESEncrypt('husen')
 *
 * @Author 何明胜
 *
 * @Created at 2018年3月19日 下午2:56:24
 *
 * @Version 1.0.0
 */

var triple_des_key_default = 'IwL1EObaoKTjhTN8wkuY+tXA';

/** 3DES加密,使用默认密钥 */
$.extend({
    'tripleDESEncrypt': function (dataSource) {
        return $.tripleDESEncryptByKey(dataSource, triple_des_key_default);
    }
});

/** 3DES加密，使用自定义密钥 */
$.extend({
    'tripleDESEncryptByKey': function (message, key) {
...
/** 3DES解密,使用默认密钥 */
$.extend({
    'tripleDESDecrypt': function (dataSource) {
        return $.tripleDESDecryptByKey(dataSource, triple_des_key_default);
    }
});

/** 3DES解密，使用自定义密钥 */
$.extend({
    'tripleDESDecryptByKey': function (ciphertext, key) {
        var keyHex = CryptoJS.enc.Utf8.parse(key);
