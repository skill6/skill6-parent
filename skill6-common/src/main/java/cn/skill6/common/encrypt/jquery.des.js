/**
 * @Desc DES加密工具, 使用方法：必须导入JQuery,然后使用$.方法名(参数...)调用，如$.DESEncrypt('husen')
 *
 * @Author 何明胜
 *
 * @Created at 2018年3月19日 下午2:56:11
 *
 * @Version 1.0.0
 */

var des_key_default = 'IwL1EOba';

/** DES加密,使用默认密钥 */
$.extend({
    'DESEncrypt': function (dataSource) {
        return $.DESEncryptByKey(dataSource, des_key_default);
    }
});

/** DES加密，使用自定义密钥 */
$.extend({
    'DESEncryptByKey': function (dataSource, secretKey) {
        var keyHex = CryptoJS.enc.Utf8.parse(secretKey);
        var encrypted = CryptoJS.DES.encrypt(dataSource, keyHex, {
...
/** DES解密,使用默认密钥 */
$.extend({
    'DESDecrypt': function (dataSource) {
        return $.DESDecryptByKey(dataSource, des_key_default);
    }
});

/** DES解密，使用自定义密钥 */
$.extend({
    'DESDecryptByKey': function (dataSource, secretKey) {
        var keyHex = CryptoJS.enc.Utf8.parse(secretKey);
        var decrypted = CryptoJS.DES.decrypt({
            ciphertext: CryptoJS.enc.Base64.parse(dataSource)
        }, keyHex, {
            mode: CryptoJS.mode.ECB,
            padding: CryptoJS.pad.Pkcs7
        });
